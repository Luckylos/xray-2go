...
# ── Argo inbound JSON（复用同一端口，ws/xhttp 二选一）────────
# path 统一为 /argo
_argo_inbound_json() {
...
manage_packages() {
    ...
    for package in "$@"; do
        command -v "$package" > /dev/null 2>&1 && green "${package} already installed" && continue
        yellow "正在安装 ${package}..."
...
ask_freeflow_mode() {
    ...
    case "${FREEFLOW_MODE}" in
        ...
        xhttp)       green  "已选择：VLESS+XHTTP FreeFlow（path=${FF_PATH}）"       ;;
...
install_xray() {
    ...
    # 生成初始 config.json（argo inbound 由后续流程注入）
...
EOF
    ...
    if [ "${ARGO_MODE}" = "yes" ]; then
        # 初始 tunnel.service 使用临时隧道（若协议为 xhttp 则后续 _apply_fixed_tunnel 会覆盖）
...
# ── 固定隧道配置（共用，ws/xhttp 均走同一 ARGO_PORT）────────
_apply_fixed_tunnel() {
    yellow "固定隧道回源端口为 ${ARGO_PORT}（${ARGO_PROTOCOL}），请在 CF 后台配置对应 ingress"
    echo ""
...
# ── 获取/刷新节点信息 ─────────────────────────────────────────
# 参数 $1=argodomain（可选，外部传入跳过交互）
# 参数 $2=skip_tunnel_select（"1" 跳过隧道类型选择，直接用当前 ARGO_PROTOCOL/argodomain）
get_info() {
    clear
    local IP cur_uuid
    IP=$(get_realip)
        ...
        if [ "${ARGO_MODE}" = "yes" ]; then
            if [ -z "$skip_select" ]; then
                ...
                case "${tunnel_choice}" in
                    2)
                        if [ "${ARGO_PROTOCOL}" = "xhttp" ]; then
                            yellow "⚠ XHTTP 模式不支持临时隧道，已锁定为固定隧道模式" >&2
                        fi
                        if _apply_fixed_tunnel >&2; then
...
check_and_install_cron() {
    ...
    case "${choice}" in
        n|N)
            red "未安装 cron，自动重启功能无法使用"; return 1 ;;
        *)
            yellow "正在安装 cron..."
    ...
    case "${choice}" in
        ...
        2)
            if [ "$is_fixed" != "true" ]; then
                yellow "当前为临时隧道，请先配置固定隧道再切换协议"; sleep 2; manage_argo; return
            fi
        ...
        5)
            ...
            # 更新 config.json 中该 inbound 的端口
            jq --argjson old "${ARGO_PORT}" --argjson new "$new_port" \
                '(.inbounds[] | select(.port == $old) | .port) |= $new' \
                "${config_dir}" > "${config_dir}.tmp" && mv "${config_dir}.tmp" "${config_dir}"
            # 更新 tunnel 服务中的端口引用（临时隧道场景）
            if is_alpine; then
...
manage_freeflow() {
    ...
    case "${choice}" in
        1)
            ...
            # 更新 url.txt：保留 Argo 节点，重建 FreeFlow 行
            {
                ...
                if [ "$cx" -eq 0 ]; then
                    ...
                    if [ "${ARGO_MODE}" = "yes" ] && [ "${ARGO_PROTOCOL}" = "xhttp" ]; then
                        ...
                        if _apply_fixed_tunnel; then
                            ...
                            red "固定隧道配置失败，请安装后从 Argo 管理菜单重新配置"
                        fi
                    else
                        get_info
                    fi
